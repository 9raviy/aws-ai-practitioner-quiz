...
async function testBedrockConnection() {
  ...
  try {
    ...
    const beginnerResponse =
      await bedrockService.generateQuestion(beginnerRequest);
    ...
    console.log(
      `Correct Answer: ${beginnerResponse.question.options[beginnerResponse.question.correctAnswer]}`
    );
    ...
    const intermediateResponse =
      await bedrockService.generateQuestion(intermediateRequest);
    console.log("✅ Intermediate question generated:");
    console.log(`Question: ${intermediateResponse.question.question}`);
    console.log(
      `Correct Answer: ${intermediateResponse.question.options[intermediateResponse.question.correctAnswer]}`
    );
    ...
    console.error(
      "3. Check that Claude 3.5 Sonnet model is available in your account"
    );
